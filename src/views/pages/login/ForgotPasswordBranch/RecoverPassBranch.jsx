...
const RecoverPassBranch = (props) => {
  ...
  const handleVerify = async () => {
    setIsLoading(true);
    try {
      const res = await axiosInstance.post("/store-owner-token-verification", {
        token,
      });
      setIsLoading(false);
      if (res.status !== 200) {
    ...
    const clear = setInterval(async () => {
      try {
        const res = await axiosInstance.post(
          "/store-owner-token-verification",
          {
            token,
          }
        );
  ...
  const handleChangePass = async (e) => {
    ...
    try {
      const res = await axiosInstance.post("/store-owner-change-password", {
        token,
...
export default RecoverPassBranch;
